	...
	prototype: {
		...
		showErrors: function(errors) {
			...
			if(errors) {
				...
				this.successsList = $.grep( this.successsList, function(element) {
		...
		check: function( element ) {
			...
			if ( this.objectLength(rules) )
				this.successsList.push(element);
			return true;
		},
		...
		defaultShowErrors: function() {
			...
			if (this.settings.success) {
				for ( var i = 0; this.successsList[i]; i++ ) {
					this.showLabel( this.successsList[i] );
				}
			}
			...
			$.ajax($.extend(true, {
				...
				success: function(response) {
					...
					if ( valid ) {
						...
						validator.successsList.push(element);
